...
narrativa = {
    ...
    artista_5 : {
        pt : [
            ...
            "Atravessadas por estes contágios, mastigam incertezas e devolvem os dentes que sua dureza extrai. <i>Presenças</i> ou <i>Jardim de um Novo Mundo</i> é um trabalho que entrecruza esferas da existência do íntimo com o público, uma compreensão de absurdo, um habitar dimensões entre forças agressivas que compõem o mundo e o abraçar-se à existência."],
        en : [
...
(function blockTextoMostra() {
    ...
    var divTextoPT = document.createElement('div');
    divTextoPT.setAttribute('class', 'sobre-expo pt');

    for (i = 0; i < qtdeTagsTextoPT; i++) {

        tagTextoPT = document.createElement(texto.pt[i][0]);
        tagTextoPT.innerHTML = texto.pt[i][1];
        tagTextoPT.setAttribute('class', texto.pt[i][2]);

        divTextoPT.appendChild(tagTextoPT);
    }
    document.getElementsByClassName("texto-mostra")[0].appendChild(divTextoPT);

    var divTextoEN = document.createElement('div');
    divTextoEN.setAttribute('class', 'sobre-expo en');

    for (i = 0; i < qtdeTagsTextoEN; i++) {

        tagTextoEN = document.createElement(texto.en[i][0]);
        tagTextoEN.innerHTML = texto.en[i][1];
        tagTextoEN.setAttribute('class', texto.en[i][2]);

        divTextoEN.appendChild(tagTextoEN);
    }
...
(function constructorSelecaoDivArtistas() {
    for (let i = 1; i < 52; i++) {
        ...
        // Creating Each option of artist
        divOptionArtist = document.createElement("div");
        divOptionArtist.setAttribute("class", "divOptionArtist");
        divOptionArtist.setAttribute("onclick", 'changeToArtist("artista_' + i + '")');
        ...
        artistNameOption.innerHTML = artistas[i];

        // Appending
        divOptionArtist.appendChild(artistImageOption);
...
(function constructorDivArtista() {
    ...
    while (id_artista <= 51) {
        ...
        nameArtist = document.createElement("p");
        nameArtist.innerHTML = artistas[id_artista];
        ...
        if (id_artista == 43) { // Selma Parreira
            ...
            divBottom.setAttribute("style", "display:flex");
            ...
            audioAppend.setAttribute("style", "width:calc(100% - (8* var(--margin)))");
...
var selecaoDivInnerHTML = 'Selecione um Artista...';

var iD_artistaSelecionado = location.hash.substring(9);
// iD_artistaSelecionado ---> #artista_'##'
...
function changeLang() {
    ...
    if (!pageLoaded) { // 1st time

        if (localStorage.changeToEnglish == 'true') {
            // Page will change to English in the next time
            // because of the variable value ^
            change('block', 'none');
        } else {
            // Page will change to Portuguese in the next time
            change('none', 'block');
        }

    } else { // when (pageLoaded) the function will always goes here
    ...
    function change(pt, en) {
        ...
        (function changeLabelButtons() {
            ...
            document.body.querySelector(".menu-option:nth-child(2) p").innerHTML = menuItemB;
            document.body.querySelector(".menu-option:nth-child(3) p").innerHTML = menuItemC;
...
// 2) Changing State of the app if hash == 'en' or any page
// Display block ---> .pt .en elements
if (location.hash === '#en') {
...
function changeToArtist(artistSelected) {
    ...
    // If page is loaded (changing with menu) close the menu
    if (pageLoaded) {
        menuArtists();
    }
    ...
    function change() {
        ...
        if (pageLoaded) {
            ...
            // Setting value of what been selected
            valueOptionArtistaSel = artistSelected
    
            // Setting this value on url
            document.location.hash = valueOptionArtistaSel;
    ...
    setTimeout(function() { showAgain()}, 1500);
...
function changeToPage(page) {
    ...
    if (!page) {
        ...
        // display:none all content
        numberOfBlocks = document.getElementsByClassName('block').length;
        ...
        function displayThePage(page) {
            document.getElementsByClassName("loading-animation")[0].style.display = 'none';
            // Displaying the content container
